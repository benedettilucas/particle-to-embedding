...
class XFeat(nn.Module):
	...
	def detectAndCompute(self, x, top_k = None, detection_threshold = None):
		...
		M1, K1, H1 = self.net(x) # feats, keypoints, heatmap
	...
	@torch.inference_mode()
	def computeAtKeypoints(self, x, keypoints):
		"""
		Compute descriptors at given keypoints (e.g., particle projections).

		Args:
			x: torch.Tensor(B, C, H, W) - input image
			keypoints: torch.Tensor(B, N, 2) - (x, y) coordinates em pixels
		Returns:
			List[Dict]:
				'keypoints'    -> (N, 2)
				'descriptors'  -> (N, D)
		...
		# Interpolação bilinear nos keypoints
		feats = self.interpolator(M1, keypoints / torch.tensor([rw1, rh1], device=x.device), H=_H1, W=_W1)
		feats = F.normalize(feats, dim=-1)

		# Corrige escala das coordenadas se quiser usar adiante
		keypoints = keypoints * torch.tensor([rw1, rh1], device=keypoints.device).view(1, 1, -1)
	...
	@torch.inference_mode()
	def computeGlobalDescriptor(self, x, output_dim=64, resize_to_receptive=True):
		"""
			Compute a single global descriptor for the entire image (e.g., nadir view).

			input:
				x -> torch.Tensor(B, C, H, W): grayscale or rgb images
				output_dim -> int: output descriptor dimensionality (default 256)
				resize_to_receptive -> bool: resize image to receptive field (≈325x325)
			return:
				List[Dict]:
					'keypoints'    -> torch.Tensor(1, 2): pseudo-center keypoint (x, y)
					'scores'       -> torch.Tensor(1,): dummy score = 1.0
					'descriptors'  -> torch.Tensor(1, D): global descriptor (L2 normalized)
		"""
		# === 1. Pré-processamento (mantém coerência com detectAndCompute) ===
		x, rh1, rw1 = self.preprocess_tensor(x)
		B, _, _H1, _W1 = x.shape

		# === 2. Resize opcional para receptive field da rede ===
		if resize_to_receptive:
			x = F.interpolate(x, size=(416, 416), mode='bilinear', align_corners=False)
		self.x_receptive = x
		# === 3. Forward pass na rede principal ===
		M1, K1, H1 = self.net(x)       # M1: features (B, 64, H/8, W/8)
		M1 = F.normalize(M1, dim=1)    # Normaliza canais

		# === 4. Global pooling para resumir toda a imagem ===
		global_feats = F.adaptive_avg_pool2d(M1, (1, 1)).flatten(1)  # (B, 64)

		# === 5. Projeção opcional para descritor maior (e.g. 256D) ===
		if output_dim != M1.shape[1]:
			proj = torch.nn.Linear(M1.shape[1], output_dim, bias=False).to(global_feats.device)
			global_feats = proj(global_feats)

		# === 6. L2-Normalização ===
		global_feats = F.normalize(global_feats, dim=-1)

		# === 7. Cria keypoint e score dummy (para manter formato do retorno) ===
		center_kpt = torch.tensor([[x.shape[-1] / 2, x.shape[-2] / 2]],
								device=x.device).repeat(B, 1, 1)  # (B, 1, 2)
		scores = torch.ones((B, 1), device=x.device)

		# === 8. Retorno compatível com detectAndCompute() ===
		return [
			{
				'keypoints': center_kpt[b],
				'scores': scores[b],
				'descriptors': global_feats[b].unsqueeze(0)
			}
			for b in range(B)
		]
	...
	def match_lighterglue(self, d0, d1, min_conf = 0.1):
		...
		return d0['keypoints'][idxs[:, 0]].cpu().numpy(), d1['keypoints'][idxs[:, 1]].cpu().numpy(), out['matches'][0].cpu().numpy(), out
